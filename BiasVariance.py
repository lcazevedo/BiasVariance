...
def main_sequence():
    ...
    log.info(f"The settings have been read from {runtimes['load_config']}.")
    ...
    log.info(f"Model has been read.")

    #decomposition <- kfold_bias_variance_decomp
    decomposition, new_runtimes = kfold_bias_variance_decomp(log, config, model, ds_qm9, 
    ...
    folds_processed, decomposition = read_folds_results(config, folds, ds_qm9)
    if len(folds_processed) == config[config["test_ref"]]["num_folds"]:
        # statistical analysis
        statistics = statistical_analysis(config, decomposition, dataQM9)
        log.info(f"Statistics has been calculated.")

        runtimes["total run time"] = str(datetime.now() - t0)
        
        #save_results    
        save_results(log, decomposition, dataQM9, runtimes, statistics, config)
    else:
        log.info(f"You have {folds_processed} folds processed. You need all {config[config['test_ref']]['num_folds']} folds processed to calculate statistics and save results.")
...
def kfold_bias_variance_decomp(log, config, model, ds, folds, fold_to_process):
    ...
    for fold in folds_to_process:
        ...
        error[test], bias[test], var[test], predictions[test], new_runtimes = bias_variance_decomp(log, test_config, fold,
                                                                  model,
                                                                  ds_train,
                                                                  ds_test,
        ...
        save_fold_results(config, fold, error, bias, var, predictions)
   
    runtimes.update({"total_kfold_bias_variance_decomp": str(datetime.now() - t)})
        
    return (error, bias, var, predictions), runtimes
...
def exec_range(num_round, num_rounds, log, test_config, ds_train, model, column_y, ds_test):
        try:
            ...
            ds_boot = _draw_bootstrap_sample(log, test_config, ds_train)
            ...
            log.info("erro com tipo de variável")  
              
        return (num_round, pred)
...
def bias_variance_decomp(log, test_config, count_fold, model, ds_train, ds_test, 
    ...
    runtimes.update({"total_bias_variance_decomp": str(datetime.now() - t)})
    
    return loss, bias, var, np.transpose(all_pred), runtimes


def _draw_bootstrap_sample(log, test_config, ds_train):
    ...
    ds_boot = Subset.from_dataset(ds_train, bootstrap_indices)
    return ds_boot
...
def save_fold_results(config, fold, error, bias, var, predictions):
...
def read_folds_results(config, folds_train_test, ds):
    ...
    return folds_processed, decomposition
...
def load_cofigurations():
    ...
    # validate the number of folds to process
...
def read_qm9(log, config):
    ...
    if os.path.isfile(file_name):
        ...
        kf = KFold(config[config['test_ref']]["num_folds"], shuffle=True, random_state=5)
        folds = {}
        count_fold = 1
...
def save_results(log, decomposition, dataQM9, runtimes, statistics, config):
